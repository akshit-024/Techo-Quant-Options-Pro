...
describe("Sprint 4 integrated workflows", () => {
  it("keeps the operational decision fail-closed on demo data", async () => {
    const user = userEvent.setup();
    render(<App />);
    await useDemoWorkspace(user);

    const gate = screen.getByRole("complementary", {
      name: "Operational decision gate",
    });
    expect(gate).toHaveTextContent("NO TRADE");
    expect(gate).toHaveTextContent(
      "Demo data cannot generate an operational signal",
    );
    expect(
      screen.getByText("DEMO ANALYTICAL DECISION"),
    ).toBeInTheDocument();
  });
  ...
  it("makes no backend request when the API base URL is not configured", async () => {
    ...
    try {
      ...
      expect(
        screen.getByText(/No network request has been made/),
      ).toBeInTheDocument();
  ...
  it("calculates whole-lot affordability and shows the mandated zero-lot state", async () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: "Position sizing and trade plan",
      }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByText(/0 lots:/).length,
  ...
  it("shows execution modes without allowing live activation or exposing a browser secret", async () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: "Settings and guardrails",
      }),
    ).toBeInTheDocument();

    expect(
      screen.getByRole("radio", { name: /Live automatic/i }),
    ).toBeDisabled();

    expect(
      screen.getByText("LIVE_AUTOMATIC remains locked"),
    ).toBeInTheDocument();

    expect(
      screen.queryByRole("textbox", {
        name: /api key|token|secret/i,
      }),
